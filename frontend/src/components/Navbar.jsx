      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden md:flex items-center gap-2">
          ...
          {user && (
            <>
              {role === 'admin' && (
                <NavLink to="/dashboard" className={linkClass}>Admin</NavLink>
              )}
              {role === 'horse_owner' && (
                <NavLink to="/horse-owner/dashboard" className={linkClass}>Chủ ngựa</NavLink>
              )}
              {(role === 'referee' || role === 'race_referee') && (
                <NavLink to="/referee/dashboard" className={linkClass}>Trọng tài</NavLink>
              )}
              {role === 'jockey' && (
                <NavLink to="/jockey" className={linkClass}>Nài ngựa</NavLink>
              )}
            </>
          )}
        </nav>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-gray-200">
          <div className="px-4 pt-2 pb-3 space-y-1">
            <NavLink to="/" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Trang chủ</NavLink>
            <NavLink to="/tournaments" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Giải đấu</NavLink>
            <NavLink to="/predictions" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Dự đoán</NavLink>

            {user && (
              <>
                {role === 'admin' && (
                  <NavLink to="/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Admin Portal</NavLink>
                )}
                {role === 'horse_owner' && (
                  <NavLink to="/horse-owner/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Chủ ngựa</NavLink>
                )}
                {(role === 'referee' || role === 'race_referee') && (
                  <NavLink to="/referee/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Trọng tài</NavLink>
                )}
                {role === 'jockey' && (
                  <NavLink to="/jockey" className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 hover:text-slate-900 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>Nài ngựa</NavLink>
                )}
                <button
                  onClick={handleLogout}
