...
export default function JockeyOverview() {
  const { user } = useAuth()
  const api = useApi()
  const [stats, setStats]     = useState({ total_races:0, wins:0, active_horses:0, upcoming:0, win_rate:0, license_number:'—' })
  const [upcoming, setUpcoming] = useState([])

  useEffect(() => {
    api.get('/jockey/stats').then(d => { if (d?.success) setStats(d.data) })
    api.get('/jockey/races/upcoming').then(d => { if (d?.success) setUpcoming(d.data) })
  }, [])

  return (
    <div className="flex-1 ml-64 pt-16" style={{background:'#131315', minHeight:'100vh'}}>
      <header className="fixed top-0 left-64 right-0 h-16 z-40 flex items-center justify-between px-6" style={{background:'rgba(19,19,21,0.95)', borderBottom:'1px solid #3d4a3b'}}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{border:'2px solid #5bf06c', background:'#2a2a2c'}}>
            <span style={{color:'#5bf06c', fontWeight:700, fontSize:14}}>{user?.full_name?.charAt(0)}</span>
          </div>
        </div>
      </header>
      ...
      <main className="p-10 space-y-10 fade-in">
        <section style={{background:'#2C2C2E', padding:32, borderRadius:4, borderLeft:'4px solid #5bf06c'}}>
          <h2 style={{fontFamily:'Montserrat', fontWeight:800, fontSize:40, color:'#fff', letterSpacing:'-0.02em', marginBottom:12}}>
            Xin chào, {user?.full_name || 'Jockey'}
          </h2>
          <div style={{display:'flex', alignItems:'center', gap:12}}>
            ...
            <span style={{display:'flex', alignItems:'center', gap:6, color:'#bccbb6', fontSize:13}}>
              <span className="live-dot" style={{width:8, height:8, borderRadius:'50%', background:'#5bf06c', display:'inline-block'}}/>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <div key={i} style={{background:'#2C2C2E', padding:24, borderRadius:4, border:'1px solid #3d4a3b', transition:'border-color 0.2s'}}
              ...
              {upcoming.map(r => (
                <div key={r.id} style={{background:'#2C2C2E', padding:24, borderRadius:4, border:'1px solid #3d4a3b', display:'flex', gap:24}}>
                  <div style={{width:80, height:80, flexShrink:0, background:'#353437', borderRadius:4, border:'1px solid #3d4a3b', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center'}}>
                    <span style={{fontFamily:'Montserrat', fontWeight:700, fontSize:24, color:'#5bf06c'}}>{new Date(r.race_date).getDate().toString().padStart(2,'0')}</span>
                    <span style={{fontSize:10, fontWeight:700, color:'#bccbb6', textTransform:'uppercase', letterSpacing:'0.1em'}}>TH{new Date(r.race_date).getMonth()+1}</span>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:8}}>
                      ...
                      <Badge status={r.reg_status === 'approved' ? 'confirmed' : r.status || 'scheduled'} />
                    </div>
