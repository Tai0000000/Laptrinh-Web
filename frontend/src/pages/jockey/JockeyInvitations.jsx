                ...
                <div style={{padding:32, display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
                  ...
                  <div style={{display:'flex', gap:16, paddingTop:24, borderTop:'1px solid #3d4a3b', marginTop:24}}>
                    <button disabled={!!responding} onClick={() => respond(inv.id,'accepted')}
                      style={{flex:1, padding:14, background:'#5bf06c', color:'#00390c', fontFamily:'Montserrat', fontWeight:800, fontSize:14, border:'none', borderRadius:2, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>
                      <Icon name="check_circle" fill size={18}/>Chấp nhận
                    </button>
                    <button disabled={!!responding} onClick={() => respond(inv.id,'rejected')}
                      style={{flex:1, padding:14, background:'transparent', color:'#ffb4ab', fontFamily:'Montserrat', fontWeight:800, fontSize:14, border:'1px solid #ffb4ab', borderRadius:2, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>
                ...
                {history.length === 0 ? (
                  ...
                  <tr key={row.id} style={{borderBottom:'1px solid rgba(61,74,59,0.4)'}}>
                    ...
                    <td style={{padding:'16px 24px', textAlign:'right'}}>
                      {row.result ? <span style={{fontFamily:'Montserrat', fontWeight:700, color:'#5bf06c'}}>{row.result}</span> : <span style={{color:'#bccbb6'}}>—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
