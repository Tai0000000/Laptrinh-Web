...
export default function JockeyPerformance() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/jockey/performance/results'),
      api.get('/jockey/performance/best-times'),
    ]).then(([rRes, bRes]) => {
      if (rRes.data?.success) setResults(rRes.data.data ?? []);
      if (bRes.data?.success) setBestTimes(bRes.data.data ?? []);
    }).catch(console.error)
          ...
          <>
            ...
            <div style={{ background: '#2C2C2E', padding: 24, borderRadius: 4, border: '1px solid #3d4a3b', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <p style={{ fontFamily: 'Montserrat', fontWeight: 800, fontSize: 36, color: '#5bf06c' }}>
                  {totalPrize.toLocaleString('vi-VN')} ₫
                </p>
              </div>
