...
const DetailRow = ({ label, value, highlight }) => (
  <div className="flex justify-between items-start gap-4">
    ...
    <span
      className={`text-sm font-semibold text-right ${highlight ? "text-indigo-600" : "text-slate-700"}`}
    >
      {value}
    </span>
  </div>
...
const rankBadge = (rawRank) => {
  ...
  if (rawRank === 2)
    return { text: "Hạng 2", cls: "bg-slate-100 text-slate-700" };
  if (rawRank === 3)
    return {
      text: "Hạng 3",
...
const ResultsRewards = () => {
    ...
    api
      ...
      .then(async (hRes) => {
        ...
        const resultsArrays = await Promise.all(
          horses.map((h) =>
            api
              .get(`/horse-owner/horses/${h.id}/results`)
              .then((r) =>
                (r.data?.data ?? r.data ?? []).map((row) => ({
                  ...row,
                  horse_name: h.name,
                })),
              )
        ...
        setResults(
          resultsArrays
            ...
            .sort(
              (a, b) =>
                new Date(b.race_date ?? b.created_at) -
                new Date(a.race_date ?? a.created_at),
            ),
        );
      })
  ...
  const displayed =
    filter === "wins"
      ? results.filter((r) => parseInt(r.finish_position ?? r.rank) === 1)
      : results;
  const wins = results.filter(
    (r) => parseInt(r.finish_position ?? r.rank) === 1,
  ).length;
  const totalPrize = results.reduce(
    (s, r) => s + (parseFloat(r.prize_amount) || 0),
    0,
  );
      ...
      <div className="max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Thành tích thi đấu của đàn ngựa
          </p>
        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-gray-700 text-sm">
                          {r.race_name ?? `Cuộc đua #${r.race_id}`}
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold ${posColor(pos)}`}
                          >
                            {posLabel(pos)}
                          </span>
                        </td>
