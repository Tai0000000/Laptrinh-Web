    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-7">
        ...
        <p className="text-gray-500 text-sm mb-6">
          Chọn ngựa và cuộc đua để đăng ký tham gia
        </p>
              ...
              {races
                ...
                .map((r) => (
                  <option key={r.id} value={r.id}>
                    ...
                    {r.race_time
                      ? ` (${new Date(r.race_time).toLocaleDateString("vi-VN")})`
                      : ""}
                  </option>
                ))}
            </select>
...
const RaceRegistrations = () => {
    ...
    api
      ...
      .then(async (hRes) => {
        ...
        const scheduleArrays = await Promise.all(
          horseList.map((h) =>
            api
              .get(`/horse-owner/horses/${h.id}/schedule`)
              .then((r) => {
  ...
  const handleCancel = async (reg) => {
    ...
    if (!regId) {
      alert("Không tìm thấy ID đăng ký.");
      return;
    }
    if (
      !window.confirm(`Hủy đăng ký ngựa "${reg.horse_name}" khỏi cuộc đua này?`)
    )
      return;
    setCancelling(regId);
    try {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Quản lý việc đăng ký tham gia của ngựa bạn
            </p>
          </div>
          <button
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-16 text-center">
                      ...
                      <p className="text-gray-500 text-sm">
                        Chưa có đăng ký nào. Hãy đăng ký cuộc đua cho ngựa của
                        bạn!
                      </p>
                    </td>
                  </tr>
                  ...
                  registrations.map((reg, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-gray-500 text-sm">
                          {reg.race_date
                            ? new Date(reg.race_date).toLocaleDateString(
                                "vi-VN",
                              )
                            : "—"}
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-semibold ${s.cls}`}
                          >
                            {s.label}
                          </span>
                        </td>
