...
class JockeyController extends Controller
{
    ...
    {
        return $request->user()->load('jockeyProfile');
    }

    private function ok($data): \Illuminate\Http\JsonResponse
    {
    ...
    public function stats(Request $request)
    {
        ...
        $regs  = Registration::where('jockey_id', $user->id)->get();
        $total = $regs->count();

        $wins = RaceResult::whereHas(
            'registration', fn($q) => $q->where('jockey_id', $user->id)
        )->where('rank', 1)->count();

        $activeHorses = $regs->where('status', 'confirmed')
            ->pluck('horse_id')->unique()->count();

        $upcoming = Registration::where('jockey_id', $user->id)
        ...
        return $this->ok([
            ...
            'license_number' => '—',  // bảng jockeys chưa có cột license_number
        ]);
    }
    ...
    public function upcomingRaces(Request $request)
    {
        ...
        $rows = Registration::with(['race.tournament', 'horse.owner'])
            ...
            ->map(fn($r) => [
                'id'          => $r->id,
                'race_date'   => $r->race->race_time,       // frontend expects race_date
    ...
    public function invitationsPending(Request $request)
    {
        ...
        $rows = HorseJockey::with(['horse.owner', 'race.tournament'])
            ...
            ->map(fn($inv) => [
                ...
                'age'         => $inv->horse->age ?? '—',
                'weight'      => '—',  // bảng horses chưa có cột weight
    ...
    public function performanceResults(Request $request)
    {
        ...
        $rows = RaceResult::with(['registration.race.tournament', 'registration.horse'])
            ->whereHas('registration', fn($q) => $q->where('jockey_id', $user->id))
            ->orderByDesc('created_at')
            ->get()
            ->map(fn($r) => [
                'id'              => $r->id,
