...
class RaceController extends Controller
{
    ...
    // GET /api/races  — public, dùng cho Spectator chọn race để đặt cược
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => $races]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($tournament) {
            ...
            $startDate = (new \DateTime($tournament->start_date))->setTime(0, 0, 0);
            $endDate   = (new \DateTime($tournament->end_date))->setTime(23, 59, 59);

            if ($raceDate < $startDate || $raceDate > $endDate) {
                return response()->json([
                    'message' => 'Ngày đua phải nằm trong thời gian diễn ra giải đấu ('
                        . date('d/m/Y', strtotime($tournament->start_date)) . ' đến '
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        if ($race->status === 'finished') {
            return response()->json(['message' => 'Không thể xóa cuộc đua đã hoàn thành.'], 400);
        }
    ...
    // GET /api/races/live  — public
    public function live(): JsonResponse
    {
        $liveRaces = $this->raceService->getLiveRaces();
        return response()->json(['success' => true, 'data' => $liveRaces]);
    }
}
